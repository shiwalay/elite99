...
import { ArrowLeft, Clock, Calendar, Sparkles, AlertCircle, CheckCircle, Share2, ThumbsUp, Bookmark, Copy, Award } from "lucide-react";
...
export default function BlogPostDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [activeArticle, setActiveArticle] = useState<PostContent | null>(null);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        foundPost = parsed.find((p: any) => p.slug === slug);
      } catch (e) {
        // ignore
      }
    }
    ...
    if (!foundPost) {
      foundPost = defaultPosts.find((p: any) => p.slug === slug);
    }
    ...
    if (foundPost) {
      ...
      const paragraphs = foundPost.content ? foundPost.content.split("\n\n").filter((p: string) => p.trim().length > 0) : [];
      ...
      setActiveArticle({
        ...
        conclusion: "Deploying structured platforms and automation triggers is the key to building authority and scaling operations.",
        content: foundPost.content
      });

      // Load related recommendations
      const filtered = defaultPosts.filter((p) => p.slug !== slug);
      const matched = filtered.filter((p) => p.category === foundPost.category).slice(0, 2);
      const final = matched.length === 2 ? matched : filtered.slice(0, 2);
      setRelatedPosts(final);
  ...
  const renderParagraph = (text: string, pIdx: number) => {
    ...
    if (trimmed.startsWith("- [ ]") || trimmed.startsWith("- [x]") || trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
      ...
      return (
        <ul key={pIdx} className="space-y-3 my-6 pl-5 list-disc text-slate-300">
          {items.map((item, iI) => (
    ...
    return (
      <p key={pIdx} className="text-slate-300 text-base sm:text-[18px] leading-[1.8] font-light my-6 tracking-wide">
        {trimmed}
      </p>
    );
  };
  ...
  // Get raw body array
  const rawParagraphs = activeArticle.content ? activeArticle.content.split("\n\n").filter(p => p.trim().length > 0) : [];
    ...
    <div className="py-12 pb-20 relative">
      ...
      <div className="max-w-[720px] mx-auto px-4 sm:px-0 mb-8 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-650 font-medium">
          ...
          <span>Insights</span>
          <span>/</span>
          <span className="text-[#d4af37]">{activeArticle.category}</span>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <button className="text-[10px] font-bold text-[#d4af37] hover:text-[#f3e5ab] uppercase tracking-wider">Follow</button>
              </div>
              <p className="text-slate-550 text-[11px] font-medium">Systems Architect &amp; AI Consultant &middot; <span className="font-mono">{activeArticle.date}</span></p>
            </div>
          </div>

          {/* Social shares and bookmarks */}
          <div className="flex items-center gap-4 text-slate-500">
            <span className="text-xs font-mono">{activeArticle.readTime}</span>
            <span className="text-slate-800">|</span>
            <button onClick={handleCopyLink} className="hover:text-white transition-colors" title="Copy Link">
            ...
            <button onClick={toggleBookmark} className="hover:text-amber-500 transition-colors" title="Bookmark Story">
      ...
      <article className="max-w-[720px] mx-auto px-4 sm:px-0 text-slate-200">
        ...
        <div className="pt-8 border-t border-slate-900 mt-12 space-y-3 bg-[#050e1c]/45 p-6 rounded-2xl border border-slate-900/60">
        ...
        <div className="flex items-center justify-between border-y border-slate-900/65 py-4 my-10 text-slate-500">
          <div className="flex items-center gap-6">
            <button 
              onClick={handleClap} 
              className={`flex items-center gap-2 hover:text-[#d4af37] transition-all ${hasClapped ? 'text-[#d4af37] scale-105' : ''}`}
          ...
          <div className="flex items-center gap-4">
            <button onClick={toggleBookmark} className="hover:text-white transition-colors" title="Bookmark story">
              <Bookmark className={`h-4.5 w-4.5 ${isBookmarked ? 'fill-[#d4af37] text-[#d4af37]' : ''}`} />
            </button>
            <button onClick={handleCopyLink} className="hover:text-white transition-colors" title="Share story link">
        ...
        <div className="glass-panel border-slate-800 p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-[#050e1c] to-[#02070f] flex flex-col sm:flex-row gap-6 items-center my-12">
          ...
          <div className="max-w-[720px] mx-auto px-4 sm:px-0 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {relatedPosts.map((post) => (
                <div key={post.slug} className="glass-panel border-slate-900 hover:border-slate-800 p-5 rounded-xl flex flex-col justify-between space-y-4 group">
                  <div className="space-y-2">
                    {post.featuredImage && (
                      <div className="relative w-full h-32 overflow-hidden rounded-lg mb-2">
                        <img src={post.featuredImage} alt={post.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-102" />
                      </div>
                    )}
                    <span className="text-[10px] uppercase text-[#d4af37] font-bold tracking-wider">{post.category}</span>
                    <Link href={`/blog/${post.slug}`} className="block">
                      <h5 className="text-white font-bold text-base group-hover:text-[#d4af37] transition-colors leading-snug line-clamp-2">{post.title}</h5>
                    </Link>
                  </div>
                  <div className="flex justify-between items-center pt-2 border-t border-slate-950/60 text-[11px] text-slate-500">
                    <span className="font-mono">{post.date}</span>
                    <Link href={`/blog/${post.slug}`} className="text-[#d4af37] font-bold hover:underline">Read &rarr;</Link>
                  </div>
                </div>
              ))}
            </div>
